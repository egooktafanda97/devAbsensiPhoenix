...
class Forgot extends CI_Controller {
	...
	public function index(){
		if($this->session->userdata('userSession')){				
        	...
        	$this->load->view('loginregister/forgot');	
		}
	}
	...
	function checkEmail(){
        ...
        if ($this->form_validation->run() == FALSE){
        	...
        	$checkEmail1  = $this->Agivest_model->checkEmailValidForgot("where username='$username' AND email = '$email'");
        	if($checkEmail1 == 0){
	            $this->session->set_flashdata('gagalmasuk','Email belum terdaftar');  
	            $this->load->view('loginregister/forgot');
            }else{
                $emailVerif = $this->Agivest_model->checkEmailValid2($email,$username);
                $result2 = '';
                ...
                $this->Agivest_model->updateStatus($verificationCode,$email, $username);
                //Send Email
                ...
                $this->load->view('loginregister/forgot', $data);
            } 
        }
	}
