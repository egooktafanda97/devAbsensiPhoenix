  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer" style="border:none!important;">
        <button type="button" id="btnProc" class="btn btn-primary" onclick="addDataPengaturan();">
            SIMPAN
            </button>
      </div>
    </div>
...
<script type="text/javascript">
   ...
   function getPengaturan() {
    ...
    function managePengaturan(str){
        ...
        if(str=='add') {
            ...
            const save = async () => {
                const posts = await axios.get('<?= api_url(); ?>pengaturan-instansi/get-by-id/'+str, {
                    headers: {
                        'Authorization': 'Bearer ' + sessionStorage.getItem('_token')
                    }
                }).catch((err) => {
   ...
   function addDataPengaturan(){
       ...
       $('#btnProc').css('cursor','not-allowed');
        ...
        let timeStartSplitted = time_start.split(":");
        let timeEndSplitted = time_end.split(":");
        ...
        form_data.append('time_end', timeEndSplitted[0]+':'+timeEndSplitted[1]);
        ...
        if(statsproc=='add'){
            ...
            url = '<?= api_url(); ?>pengaturan-instansi/update/'+idsetter;
        }
