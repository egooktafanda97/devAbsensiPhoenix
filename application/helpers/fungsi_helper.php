...
function upload_gambar($imges, $url)
{
...
{
    return base_url('rest/api/' . $url);
}


function curl_request($url, $header, $data = null)
{
    $ch = curl_init();
    $headers  = $header;
    ...
    return  ["result" => json_decode($result, true), "status" => json_decode($statusCode, true)];
}

function curl_request_noheader($url, $data = null)
{
    $ch = curl_init();
    $headers  = [
...
function curl_no_post($url, $token)
{
    $token = $token;
    $ch = curl_init($url);
