...
Class Agivest_model extends CI_Model {
	...
	function getDataLoginPenduduk($username){
		$query = $this->db->query("select * from penduduk where (NIK='$username')");
	    if($query->num_rows() > 0){
			return $query->result();
	    }else{
			return false;
	...
	function getDataLoginPendudukUsername($username){
		$query = $this->db->query("select * from penduduk where (username='$username')");
	    if($query->num_rows() > 0){
			return $query->result();
	    }else{
			return false;
	...
	function getDataCheckLogined($username, $password){
		$query = $this->db->query("select * from penduduk,penduduk_orangtua WHERE penduduk.NIK=penduduk_orangtua.NIK AND (penduduk.NIK='$username' && penduduk_orangtua.namaIbu='$password')");
	    if($query->num_rows() > 0){
			return $query->result();
	    }else{
			return false;
	...
	function getDataCheckLoginedWithUsername($username, $password){
	    $password = md5($password);
		$query = $this->db->query("select * from penduduk WHERE (penduduk.username='$username' && password='$password')");
	    if($query->num_rows() > 0){
			return $query->result();
	    }else{
			return false;
	...
	function checkEmailValid2($email,$username){
		$result = $this->db->query("select * from user where email='$email' and username='$username'");
		return $result;
	}
